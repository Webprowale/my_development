...
import {
  ...
  PastBookingColumns,
} from "@/components/dashboard/applications/flight/Columns";
import tableData from "@/components/dashboard/applications/flight/data.json";
import PastBookings from "@/components/dashboard/applications/flight/PastBookings";
import {
  cancelData,
  pastData,
} from "@/components/dashboard/applications/flight/data";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import CancelledBookings from "@/components/dashboard/applications/flight/CancelledBookingns";
...
const NextReservation = () => {
  return (
    <div className="overview min-h-[250px] flex flex-col justify-between relative p-4 bg-white overflow-hidden">
      <h3 className="font-medium text-[0.75rem] md:text-[15px]">Next Flight</h3>

      <div className="overview-data flex flex-col items-start justify-between text-[#647995] text-[12px] mt-5">
        <Image
          src={"/assets/settings/qatar_logo.png"}
          width={80}
          height={80}
        ...
        <div className="reserve-cta flex mt-3 items-center justify-stretch w-full gap-1">
          <Link
            href={"#"}
            className="inline-flex items-center justify-center w-full bg-primary600 text-white p-2 text-sm rounded text-nowrap"
          >
            View Details
          </Link>
          <DropdownMenu>
...
const data = [
  ...
  },
  {
    ...
    number: 4,
    mark: "flights booked  increased by 2",
    percent: "+0.49%",
  },
  {
