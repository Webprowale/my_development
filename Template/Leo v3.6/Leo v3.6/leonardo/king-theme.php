...
class qa_html_theme extends qa_html_theme_base {
	...
	public function body_content() {
		...
		if ( 'home' == $this->template || 'hot' == $this->template || 'search' == $this->template || 'updates' == $this->template || 'user-questions' == $this->template || 'favorites' == $this->template || 'qa' == $this->template || 'tag' == $this->template || 'type' == $this->template || 'reactions' == $this->template || 'aifavs' == $this->template || 'pposts' == $this->template ) {
			...
			$this->output( '<span id="range-value">5</span><i class="fa-solid fa-square"></i>' );
			$this->output( '<input id="myRange" class="leo-slider" type="range" min="2" max="10" step="1" value="5" >' );
	...
	public function q_view( $q_view ) {
		...
		if ( ! empty( $q_view ) ) {
			...
			if ( null == $nsfw || qa_is_logged_in() ) {
				...
				if ('Q_HIDDEN' === $type) {
					$this->output( '<div class="ppost" data-toggle="tooltip" data-placement="top" title="' . qa_lang('kingai_lang/ppaget') . '"><i class="fa-solid fa-user-ninja"></i> ' . qa_lang('kingai_lang/ppage') . '</div>' );
				}
				$this->post_tags( $q_view, 'king-q-view' );
				$this->page_title_error($q_view);
				$blockwordspreg = qa_get_block_words_preg();
				if ( 'I' == $text2 ) {
					$this->output( '<div class="ai-content">' );
					$this->output( '<div class="post-content" id="prcontent">' );
					$this->output( qa_block_words_replace( $q_view['raw']['pcontent'], $blockwordspreg ) );
					$this->output( '</div>' );

					
					$this->output( '<button id="copyp"><i class="fa-solid fa-copy"></i> '.qa_lang('kingai_lang/copyp').'</button>' );
					$wai = qa_db_postmeta_get( $pid, 'wai' );
					$stle = qa_db_postmeta_get( $pid, 'stle' );
					if ($wai) {
						$this->output( '<div class="ai-w">'.qa_lang('kingai_lang/'.$wai).'</div>' );
					}
						

					if ($stle && 'none' !== $stle ) {
						$this->output( '<div class="ai-w">'.$stle.'</div>' );
					}
					$this->output( '</div>' );
										$np = qa_db_postmeta_get( $pid, 'nprompt' );
					if ($np) {
						$this->output( '<div class="nprompt ai-content">' );
						$this->output( '<p>' . qa_lang('kingai_lang/ai_nprompt') . '</p>' );
						$this->output( $np );
						$this->output( '</div>' );
					}
				} else {
					$this->output( '<div class="post-content" id="prcontent">' . qa_block_words_replace( $q_view['raw']['pcontent'], $blockwordspreg ) . '</div>' );
				}
				if ( 'poll' == $text2 ) {
					$this->get_poll( $pid );
				} elseif ( 'list' == $text2 ) {
	...
	public function header_right() {
		...
		$this->output( '<li>' );
			$this->output( '<a class="aisubmit" href="' . qa_path_html( 'submitai' ) . '"><i class="fa-solid fa-feather-pointed"></i>'.qa_lang('kingai_lang/aisubmit').'</a>' );
			$this->output( '</li>' );
	...
	public function q_list_item( $q_item ) {
		...
		if ( 'V' == $format ) {
			$postformat = '<a class="king-post-format" href="' . qa_path_html( 'type' ) . '"><i class="fas fa-video"></i> ' . qa_lang_html( 'main/home_video' ) . '</a>';
			$postc      = ' king-class-video';
		} elseif ( 'I' == $format ) {
			$postformat = '<a class="king-post-format" href="' . qa_path_html( 'type', array( 'by' => 'images' ) ) . '"><i class="fas fa-image"></i> ' . qa_lang_html( 'main/home_image' ) . '</a>';
			$postc      = ' king-class-image';
		} elseif ( 'N' == $format ) {
			$postformat = '<a class="king-post-format" href="' . qa_path_html( 'type', array( 'by' => 'news' ) ) . '"><i class="fas fa-newspaper"></i> ' . qa_lang_html( 'main/home_news' ) . '</a>';
			$postc      = ' king-class-news';
		} elseif ( 'poll' == $format ) {
			$postformat = '<a class="king-post-format" href="' . qa_path_html( 'type', array( 'by' => 'poll' ) ) . '"><i class="fas fa-align-left"></i> ' . qa_lang_html( 'misc/king_poll' ) . '</a>';
			$postc      = ' king-class-poll';
		} elseif ( 'list' == $format ) {
			$postformat = '<a class="king-post-format" href="' . qa_path_html( 'type', array( 'by' => 'list' ) ) . '"><i class="fas fa-bars"></i> ' . qa_lang_html( 'misc/king_list' ) . '</a>';
			$postc      = ' king-class-list';
		} elseif ( 'trivia' == $format ) {
			$postformat = '<a class="king-post-format" href="' . qa_path_html( 'type', array( 'by' => 'trivia' ) ) . '"><i class="fas fa-times"></i> ' . qa_lang_html( 'misc/king_trivia' ) . '</a>';
			$postc      = ' king-class-trivia';
		} elseif ( 'music' == $format ) {
			$postformat = '<a class="king-post-format" href="' . qa_path_html( 'type', array( 'by' => 'music' ) ) . '"><i class="fas fa-headphones-alt"></i> ' . qa_lang_html( 'misc/king_music' ) . '</a>';
			$shomag     = false;
		...
		$this->output( '</div>' );
			if ( $shomag ) {
			$this->output( '<a href="' . $q_item['url'] . '" class="ajax-popup-link magnefic-button mgbutton" data-toggle="tooltip" data-placement="right" title="' . qa_lang_html( 'misc/king_qview' ) . '"><i class="fas fa-search"></i></a>' );
		} else {
			$this->output( '<a href="' . $q_item['url'] . '" class="king-listen magnefic-button mgbutton" data-toggle="tooltip" data-placement="right" title="' . qa_lang_html( 'misc/king_listen' ) . '"><i class="fa-solid fa-headphones"></i></a>' );
		}	
	...
	public function q_item_content( $q_item ) {
		...
		if ( null !== $nsfw && ! qa_is_logged_in() ) {
			...
			if ( $text2 ) {
				...
				$this->output_raw( '<span class="post-featured-img"><img class="item-img king-lazy" data-king-img-src="' . $text . '" alt=""/></span>' );
			}
	...
	public function q_item_title( $q_item, $postformat=null ) {
		...
		$this->output( '</div>' );
		$this->output( '<A HREF="' . $q_item['url'] . '"><h4>' . $q_item['title'] . '</h4></A>' );
		$this->output( '</DIV>' );
	}
